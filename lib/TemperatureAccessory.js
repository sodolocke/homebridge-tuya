...
class TemperatureAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        this.device.on('change', (changes, state) => {
            if (changes.hasOwnProperty(this.dpCurrentTemperature) && characteristicCurrentTemperature.value !== changes[this.dpCurrentTemperature] ) this.characteristicTemperature.updateValue(changes[this.dpCurrentTemperature]);

            if (changes.hasOwnProperty(this.dpCurrentHumidity) && characteristicCurrentHumidity.value !== changes[this.dpCurrentHumidity]) this.characteristicCurrentHumidity.updateValue(changes[this.dpCurrentHumidity]);
